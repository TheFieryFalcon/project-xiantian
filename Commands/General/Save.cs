    ...
    partial class GeneralCommands {
        public static void Save(GameContext context, char[] flags, string[] parameters) {
            ...
            if (flags.Contains(char.Parse("d"))) {
                try { File.Delete($"./Saves/save{parameters[0]}.sav"); } catch { Exceptions.E1(); return;}
                ...
                if (flags.Contains(char.Parse("n"))) {
                    try { number = int.Parse(parameters[0]); } catch { Exceptions.E1(); return; } // I despise tryparse, it doesn't even check for null or index exceptions
                }
                Functions.Save(context, number);
